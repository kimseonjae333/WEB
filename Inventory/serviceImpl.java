...
import kr.ac.kopo.ctc.kopo39.dao.InventoryDao;
import kr.ac.kopo.ctc.kopo39.dao.InventoryDaoImpl;
...
public class InventoryServiceImpl implements InventoryService {
	...
	public Pagination getPagination(int C, int countPerPage) { //인자값 받기
		...
		InventoryDao inventroyDao = new InventoryDaoImpl(); //StudentScoreDaoImpl 객체 생성
		int total = inventroyDao.count(); //StudentScoreDaoImpl의 count()함수 호출하여 리턴값 total에 저장
		...
		if(startBlock == 1) { //스타트 블록이 1이면
			p = -1; //p 값 -1로 설정
			pp = -1; //pp 값 -1로 설정
			if(lastpage < 10) { //마지막 페이지가 10 이하 일때
			...
			n = currentPage + blocksize; //n 값은 현재 페이지 + 블록 사이즈(10)로 설정
				if(currentPage + blocksize > lastpage) { 
					//현재 페이지+ 블록 사이즈가 마지막 페이지 보다 크면
					n = lastpage; //n은 라스트 페이지;
				}
			}
		}
			...
			if(currentPage + blocksize > lastpage) { //현재 페이지+ 블록 사이즈가 마지막 페이지 보다 크면
				n = lastpage; //n은 마지막 페이지
			}
			nn = lastpage; //nn 값 마지막 페이지로 설정
